<?php
	...
	if(isset($_POST["anadir"])){
		...
		if(isset($producto_code) && isset($cantidad)) {
			...
			$aCarrito[$iUltimaPos]['OrderLine'] = ($iUltimaPos+1);
			
			$aCarrito[$iUltimaPos]['Product_Code'] = $producto_code;
		...
		//Imprimimos el carrito en la pagina.
		foreach ($aCarrito as $key => $value) {
			
			$sHTML .= 'Orden de Linea: ' . $value['OrderLine'] . '<br>' .
						  'ID_Producto: ' . $value['Product_Code'] . '<br>' .
						  'Cantidad: ' . $value['Cantidad'] . '<br>' .
						  'Precio/Unidad: ' . $value['Precio/Unidad'] . '<br><br>';
...
<html lang="es">
  ...
  <body>
		...
		<!-- Mostramos en la pagina su lista de productos -->
		<div>
			<?php
