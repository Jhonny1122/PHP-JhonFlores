<?php
	...
	function comp_Cantidad($id_prod,$cant,$conn){
		...
		if($cant_prod < $cant){
			...
			$stmt = $conn->prepare("SELECT * FROM ALMACENA WHERE ID_PRODUCTO=:id_prod");
			...
			for($i = 0 ; $i < count($array_numAlm) ; $i++){
				...
				echo "Num Almacen: $array_numAlm[$i], ID_PRODUCTO: $id_prod, Cantidad: $array_cantProd[$i]" . "<br>";
			...
			for($i = 0 ; $i < count($array_numAlm) ; $i++){
				...
				if($array_cantProd[$i] < $cant){
					...
					echo "La cantidad almacena $array_cantProd[$i] del almacen $array_numAlm[$i] es menor a la cantidad pedida $cant" . "<br>";
					...
					echo "Num almacen $array_numAlm[$i], tiene : $array_cantProd[$i]" . "<br>";
					
				}
				else{
					
					echo "La cantidad almacena $array_cantProd[$i] del almacen $array_numAlm[$i] es mayor a la cantidad pedida $cant" . "<br>";
					
					$total=$array_cantProd[$i]-$cant;
					
					$cant=0;
					
					echo "Ahora la cantidad de $array_cantProd[$i] es $total" . "<br>";
